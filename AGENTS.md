# React Plugin Kernel — Design Brief

Working context for this repo, for humans and coding agents alike. Read
this before proposing architecture. (`CLAUDE.md` just imports this file so
Claude Code picks it up; edit here, not there.)
...
**Status: a target, not a spec.** This is the surface we're aiming at, to
keep the layers honest. Expect it to shift once `/kernel` is written — if
the implementation argues for a different shape, the implementation wins.
Update this section rather than quietly diverging from it.
...
Toolchain: pnpm workspace, TypeScript 5.9 (pinned; 6/7 changed defaults),
vitest 4 with per-package projects. `packages/kernel/tsconfig.json` has
`lib: ["ES2022"]` and `types: []` — any DOM or Node global is a compile
...
- Imperative subject line, ≤ 72 chars, no trailing period. Body explains
  *why* and what changed at the level of behaviour, not files.
- **No AI attribution trailers.** Commits and PR descriptions carry no
  `Co-Authored-By: <assistant>` or "Generated with …" lines, whichever tool
  wrote the message. Claude Code users: `.claude/settings.json` is
  gitignored, so set it locally —
