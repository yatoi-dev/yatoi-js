...
// Distinct prefix from the React example (`yatoi-todo:*`) so both example
// apps can run against the same browser profile side by side without
// clobbering each other's storage.
const STORAGE_KEY = 'yatoi-todo-vue:todos'
...
function createTodoStore(): TodoStore {
  ...
  // stable reference in between — cheap for consumers that check identity.
  let todos = load()
 ...
 * a component. This is the composition unit React/Vue have no home for
 * (see docs/design.md "A composition unit that isn't a render-tree node").
 */
